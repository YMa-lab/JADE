...
# adata1 = adatas["151673"]
# adata2 = adatas["151674"]
...
for adata in aligned_adatalist:
    clustering(adata, reduced_dim=20, key="emb_rec", n_clusters=7, refinement=True, radius=20)
ARI = []
for adata in aligned_adatalist:
...
for k, adata in enumerate(aligned_adatalist):
    layer_to_color_map = {'Layer{0}'.format(i+1): sns.color_palette()[i] for i in range(6)}
    layer_to_color_map['WM'] = sns.color_palette()[6]
...
for niter, color in zip(niter_values, colors):
    M = model.model.Ms[0].cpu().detach().numpy()
    ...
    # pi = output
    s = cal_pairwise_alignment_score(adata1,adata2,pi)
